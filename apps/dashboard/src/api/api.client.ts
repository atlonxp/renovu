import { IEnvironment, NOVU_PRODUCT_TYPE_HEADER, OrganizationProductTypeEnum } from '@novu/shared';
import { apiHostnameManager } from '@/utils/api-hostname-manager';
import { getToken } from '@/utils/auth';
import { clearAuth } from '@/utils/self-hosted/jwt-manager';
import { APP_IDS, getCurrentAppId } from '@/utils/apps';
  ...
  try {
    ...
    const config: RequestInit = {
      ...
      headers: {
        ...
        [NOVU_PRODUCT_TYPE_HEADER]: getProductTypeHeader(),
    ...
    if (!response.ok) {
      // On 401, clear auth state and redirect to sign-in (guard against multiple simultaneous 401s)
      if (response.status === 401 && jwt && !isRedirectingTo401) {
