...
import type { AddStepMenuSelection } from "./add-step-menu";
...
export const useCanvasNodesEdges = ({
	...
	const { nodes: animatedNodes, animatingNodeIds } =
		useAnimatedNodes(targetNodes);
	...
	const addNode = useCallback(
		(
			insertIndex: number,
			selection: AddStepMenuSelection | keyof typeof NODE_TYPE_TO_STEP_TYPE,
		) => {
			const workflow = dataRef.current.workflow;
			if (!workflow) return;
			...
			const selectionType =
				typeof selection === "string"
					? NODE_TYPE_TO_STEP_TYPE[selection]
					: selection.type;
	...
	const addAiStep = useCallback(
		(insertIndex: number, aiStep: StepCreateDto) => {
			const workflow = dataRef.current.workflow;
			if (!workflow) return;
			...
			const defaultLayoutId = defaultLayout?.layoutId;
			// Seed default-shaped controls then layer the AI's controlValues on top, so any required
			// fields (e.g. email layoutId, skip flags) are still present even if the AI omitted them.
			const baseStep = createStep(
				aiStep.type,
				defaultLayoutId,
				workflow.severity,
			);
			const mergedControlValues: Record<string, unknown> = {
				...(baseStep.controlValues ?? {}),
				...((aiStep.controlValues ?? {}) as Record<string, unknown>),
			};

			const newStep: StepCreateDto = {
				name: aiStep.name || baseStep.name,
				type: aiStep.type,
			...
			insertStep(insertIndex, newNode, newStep, {
				onError: () => {
					showErrorToast("Failed to add AI-generated step");
				},
			});
		},
		[insertStep, layoutsResponse, dataRef],
	...
	const reorderSteps = useCallback(
		(
			...
			const newNodesWithPosition = recalculatePositionAndIndex(
				newNodes,
				dataRef.current.containerWidth,
			);
	...
	const selectNode = useCallback(
		(id: string, goto: "editor" | "view" = "editor") => {
			...
			if (goto === "editor") {
				...
				const isTemplateConfigurable =
					TEMPLATE_CONFIGURABLE_STEP_TYPES.includes(stepType);
