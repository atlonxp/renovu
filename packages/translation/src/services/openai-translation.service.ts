...
export class OpenAITranslationService {
	...
	async translate(request: TranslateRequest): Promise<TranslateResponse> {
		...
		try {
			...
			const { tokenized, variableMap, variables } =
				this.tokenizer.tokenize(content);

			this.logger.debug(
				`Tokenized ${variables.length} variables for translation`,
			);
			...
			if (!skipValidation) {
				...
				if (!validation.valid) {
					this.logger.warn(
						`Translation validation failed: ${this.validator.getSummary(validation)}`,
					);
				}
			}
			...
			const latencyMs = Date.now() - startTime;
			this.logger.error(
				`Translation failed after ${latencyMs}ms: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
	...
	private async withRetry<T>(
		...
		for (let attempt = 0; attempt <= maxRetries; attempt++) {
			try {
				...
				const baseDelay = DEFAULT_CONFIG.BASE_DELAY_MS * 2 ** attempt;
