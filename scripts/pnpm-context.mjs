...
/**
 * Docker build context generator for pnpm monorepos.
 *
 * This script generates a tar archive containing only the files needed to build
 * a specific package in the monorepo. It uses pnpm's workspace filtering to
 * determine dependencies and creates a structured context for efficient Docker
 * layer caching.
 *
 * Usage:
 *   pnpm pnpm-context -- apps/api/Dockerfile | docker build -
 *
 * The generated context structure:
 *   - meta/: Root config files and package.json files
 *   - deps/: Source files from dependency packages
 *   - pkg/: Source files from the target package
...
const cli = meow(
  ...
  Options
    --list-files, -l    Don't generate tar, just list files.
    --patterns, -p      Additional .gitignore-like patterns.
    --root              Path to monorepository root.
  Examples
...
await parseCli(cli).then(main).catch((err) => { throw err; });
...
async function getPackagePathsFromPnpmSelector(selector, cwd) {
  ...
  return Object.keys(projects.selectedProjectsGraph)
